...
def GetAllFromWeeklyManong(start,end,filename = 'allLists.txt'):
    ...
    for i in range(start,end):
        ...
        try:
            ...
            all_lists_with_urls = ''
            for item in all_h4:
                one_list_with_url = str(i) +':'+ item.a.get_text() +'$'+ str(item.a['href']) + '\r'
...
@click.option('--fname', type = click.STRING, help='The output file name for all content, default file name is allLists.txt.')
def new(fname):
    GetAllFromWeeklyManong(1,-1)

@GetMNWeeklyContent.command()
@click.option('--start', type = click.INT, default = 1, help='The start series number, if not set, start from the 1st series.')
@click.option('--end', type = click.INT, default = -1 , help='The end series number, if not set, end to the lastest series.')
@click.option('--fname', type = click.STRING, help='The output file name for all content, default file name is allLists.txt.')
