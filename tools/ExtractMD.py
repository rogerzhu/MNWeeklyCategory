...
@click.command()
@click.option('--fname', default='allLists.txt', type=click.STRING,
              help='The raw file name for crawling file, \
              default is allList.txt.')
@click.option('--filters', default='', type=click.STRING,
              help='Input the filers that you need, seperate by \',\',\
              if no keywords, means use all filters by default.')
def GenerateMDs(fname, filters):
    ...
    # The return value could be used to do some check, ignore here.
    DeleteAllHireInfoFromListsFile()
    count = 0
    ...
    for line in f:
        ...
        for item in filters['root']:
            ...
            if any(s in name for s in filter_words) \
               and not any(s in name for s in rejector_words):
